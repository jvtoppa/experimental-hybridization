...
void compute_repair(vector<uint32_t>& in){
	...
	std::unordered_map<uint32_t, itype> char_to_int;
	...
	itype max_d = sigma + n/min_high_frequency;
	...
	for (size_t i = 0; i < in.size(); ++i) {
        uint32_t element = in[i];
        
        if (char_to_int.find(element) == char_to_int.end()) {
            char_to_int[element] = sigma;
            A.push_back(element);
            sigma++;
        }
        T.set(i, char_to_int[element]);
    }	
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        cerr << "Usage:\n  Compression:   " << argv[0] << " c [original_file_path] < [file]\n  Decompression: " << argv[0] << " d [archive_file]\n";
        return 1;
    }
    ...
    if(mode.compare("c") == 0){
        string out = "../archive.rp";
        
        // If the original file path was provided as an argument, extract its base name
        if (argc >= 3) {
        ...
        cout << "Compressing via stdin. Output will be saved to: " << out << endl;
        ...
        if (raw_data.empty()) {
            cerr << "Error: Input data from standard input is empty." << endl;
            return 1;
        }

        string T;
        T.reserve(raw_data.size());
        for (char c : raw_data) {
            T.push_back(static_cast<unsigned char>(c));
        }

        GCIS<uint32_t> gcis_comp(T); 
        vector<uint32_t> gcis_input;
        ...
        gcis_input = gcis_comp.compress(gcis_stop, gcis_metadata);

        A.clear();
        G.clear();
        T_vec.clear();

        compute_repair(gcis_input);
        ...
        ofstream meta_out(out, ios::binary | ios::app);
        if (meta_out.is_open()) {
            uint64_t metadata_size = gcis_metadata.size();
            meta_out.write(reinterpret_cast<const char*>(gcis_metadata.data()), metadata_size * sizeof(uint64_t));
            meta_out.write(reinterpret_cast<const char*>(&metadata_size), sizeof(metadata_size));
            meta_out.close();
        } else {
            cerr << "Error: Could not append archive metadata to " << out << endl;
            return 1;
        }

        cout << "Compression completed successfully. Saved to " << out << endl;
		cerr << "Compression time: " << time_compression.count() << " ms\n";

    }
    else if(mode.compare("d") == 0)
    {
        if (argc < 3) {
            cerr << "Error: Decompression requires an archive file argument." << endl;
            return 1;
        }
        string in = argv[2]; 
        
        // Strip out ".rp" to recover the original base name for decompression output
        string final_out = in + ".decompressed";
        size_t dot = in.find_last_of(".");
        if (dot != string::npos && in.substr(dot) == ".rp") {
            final_out = in.substr(0, dot);
        }

        ifstream meta_in(in, ios::binary | ios::ate);
        if (!meta_in.is_open()) {
            cerr << "Error: Could not open archive file: " << in << endl;
            return 1;
        }

        streampos file_size = meta_in.tellg();

        uint64_t metadata_size = 0;
        meta_in.seekg(static_cast<streamoff>(file_size) - static_cast<streamoff>(sizeof(uint64_t)), ios::beg);
        meta_in.read(reinterpret_cast<char*>(&metadata_size), sizeof(metadata_size));

        vector<uint64_t> gcis_metadata(metadata_size);
        streamoff payload_offset = static_cast<streamoff>(file_size) - static_cast<streamoff>(sizeof(uint64_t)) - static_cast<streamoff>(metadata_size * sizeof(uint64_t));
        meta_in.seekg(payload_offset, ios::beg);
        meta_in.read(reinterpret_cast<char*>(gcis_metadata.data()), metadata_size * sizeof(uint64_t));
        meta_in.close();

        auto pgf = packed_gamma_file3<>(in, false);
        vector<itype> A_dec;
        vector<pair<itype,itype>> G_dec;
        vector<itype> Tc_dec;
        pgf.read_and_decompress(A_dec, G_dec, Tc_dec);

        vector<uint32_t> recovered_rp; 
        std::stack<itype> S;
        for(itype i = 0; i < Tc_dec.size(); ++i){
            S.push(Tc_dec[i]);
            while(!S.empty()){
                itype X_sym = S.top();
                S.pop();
                ...
                if(X_sym < A_dec.size()){
                    recovered_rp.push_back(A_dec[X_sym]);
                } else {
                    auto ab = G_dec[X_sym - A_dec.size()];
                    S.push(ab.second);
                    S.push(ab.first);
                }
            }
        ...
        ofstream ofs(final_out, ios::binary);
        if (!ofs.is_open()) {
            cerr << "Error: Could not open output path: " << final_out << endl;
            return 1;
        }
        ofs.write(reconstructed_string.data(), reconstructed_string.size());
        ofs.close();

        cout << "Decompression done. Saved to " << final_out << endl;
    }
